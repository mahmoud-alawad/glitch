...
import { LoginCredentialsDto } from 'server/auth/dto/login-credentials.dto';
import { UserInputExceptionHandler } from 'server/common/interceptors/handlers/user-input-exception.handler';
import { hashConfig } from 'server/config/hash.config';
...
export class UserService {
  ...
  private async validateCurrentPassword(
    ...
    if (!isCorrectPassword) {
      throw new UserInputExceptionHandler();
    }
  }
   ...
   */
  async validateLogin(email: string, currentPassword: string,) {
    const user = await this.prisma.user.findUnique({ where: { email } });

    if (!user) throw new InvalidPasswordUpdateException().empltyCredentials(user.email);
    ...
    return new InvalidPasswordUpdateException().empltyCredentials(user.email);
  }
}
